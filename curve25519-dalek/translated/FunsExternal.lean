...
axiom
  ...
  Clause2_IntoIter) :
  I → J → Result (Option edwards.EdwardsPoint)
...
axiom
  ...
  Clause4_IntoIter) :
  backend.serial.scalar_mul.precomputed_straus.VartimePrecomputedStraus → I
    → J → K → Result (Option edwards.EdwardsPoint)
...
axiom
  ...
  I → J → Result edwards.EdwardsPoint
...
axiom
  ...
  core.iter.traits.collect.IntoIterator K (Option edwards.EdwardsPoint)
  Clause4_IntoIter) :
  edwards.VartimeEdwardsPrecomputation → I → J → K → Result (Option
...
axiom
  ...
  (coreborrowBorrowClause0_ItemEdwardsPointInst : core.borrow.Borrow
  Clause0_Item edwards.EdwardsPoint) :
  I → Result edwards.VartimeEdwardsPrecomputation
...
/-- [curve25519_dalek::ristretto::{impl core::iter::traits::accum::Sum<T> for curve25519_dalek::ristretto::RistrettoPoint}::sum]:
    ...
    Visibility: public -/
axiom ristretto.RistrettoPoint.Insts.CoreIterTraitsAccumSum.sum
  ...
  :
  I → Result ristretto.RistrettoPoint
...
axiom
  ...
  I → J → Result ristretto.RistrettoPoint
...
axiom
  ...
  Clause2_IntoIter) :
  I → J → Result (Option ristretto.RistrettoPoint)
...
axiom
  ...
  Clause4_IntoIter) :
  ristretto.VartimeRistrettoPrecomputation → I → J → K → Result (Option
...
axiom
  ...
  I → Result ristretto.VartimeRistrettoPrecomputation
